...
class LandlordDashboardController extends Controller
{
    ...
    private function getRecentActivitiesInternal(Request $request, array $context)
    {
        ...
        $formattedActivities = $activities->take($limit)->map(function ($item) {
            ...
            if ($item instanceof \App\Models\PaymentTransaction) {
                ...
                $status = $isPending ? 'pending' : ($isRefund ? 'refunded' : ($transactionStatus !== '' ? $transactionStatus : 'confirmed'));
                return [
                    'id' => $item->id, 'type' => 'payment',
    ...
    private function getUpcomingPaymentsInternal(Request $request, array $context)
    {
        $assignedPropertyIds = ($context['is_caretaker'] && $context['assignment']) ? $context['assignment']->getAssignedPropertyIds() : null;
    ...
    private function getPropertyPerformanceInternal(Request $request, array $context)
    {
        $assignedPropertyIds = ($context['is_caretaker'] && $context['assignment']) ? $context['assignment']->getAssignedPropertyIds() : null;
