    ...
    <div className={`bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-gray-300 dark:border-gray-700 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col ${className}`} onClick={onClick}>
      <div className="relative h-48">
        {url ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 flex items-center justify-center">
            <Users className="w-16 h-16 text-gray-500 dark:text-gray-500" />
          </div>
        )}
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        {room.tenants && room.tenants.length > 0 ? (
          ...
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 mb-4 border border-gray-200 dark:border-gray-700">
            <p className="text-[10px] font-bold text-gray-500 dark:text-gray-500 uppercase mb-2">Current Occupant(s)</p>
            <p className="font-semibold text-gray-800 dark:text-gray-200 text-sm">{room.tenant}</p>
          </div>
        ) : (
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 mb-4 border border-dashed border-gray-300 dark:border-gray-700 text-center">
        ...
        {room.amenities && room.amenities.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {room.amenities.slice(0, 6).map((a, i) => (
              <span key={i} className="px-2.5 py-2 rounded-lg bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 text-[10px] font-bold uppercase border border-green-100 dark:border-green-800/50">{typeof a === 'string' ? a : (a.name || a.title || '')}</span>
            ))}
          </div>
        )}
