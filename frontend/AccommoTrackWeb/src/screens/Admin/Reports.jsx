    ...
    <div className="w-full max-w-full px-6 py-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Review and resolve property flags from tenants.</p>
        </div>
      </div>
                ...
                <tr key={report.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-gray-900">{report.reporter?.first_name} {report.reporter?.last_name}</p>
                  ...
                  <td className="px-6 py-4 text-center">
                    <button 
                      ...
                      className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-xs font-bold transition-all shadow-sm shadow-blue-100"
                    >
                      Review
                    </button>
                  </td>
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="font-bold text-gray-900">{selectedReport.reporter?.first_name} {selectedReport.reporter?.last_name}</p>
                  <p className="text-xs text-gray-500">{selectedReport.reporter?.email}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Property Flagged</p>
                  <p className="font-bold text-gray-900">{selectedReport.property?.title}</p>
                  <p className="text-xs text-blue-600 hover:underline cursor-pointer" onClick={() => window.open(`/properties/${selectedReport.property_id}`, '_blank')}>View Listing</p>
                </div>
              </div>

              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Subject</p>
                <div className="p-3 bg-red-50 text-red-700 rounded-lg font-bold text-sm inline-block">
                  {selectedReport.reason}
                </div>
              </div>

              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Detailed Report</p>
                <div className="p-4 bg-gray-50 rounded-xl text-sm text-gray-700 whitespace-pre-wrap italic leading-relaxed border border-gray-100">
                  "{selectedReport.description}"
                </div>
              </div>

              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Admin Actions & Notes</p>
                <textarea
                  value={adminNotes}
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-between gap-3">
              <div className="flex gap-2">
                <button 
                  ...
                  className="px-6 py-2 bg-white border border-gray-200 text-gray-600 font-bold text-sm rounded-lg hover:bg-gray-100"
                >
                  Close
                </button>
              </div>
                ...
                {selectedReport.status === 'pending' && (
                  <>
                    <button
                      ...
                      className="px-6 py-2 text-gray-500 hover:text-gray-700 font-bold text-sm rounded-lg"
                    >
                      Dismiss Report
                    </button>
                    <button
                      ...
                      disabled={actionLoading}
                      className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm rounded-lg shadow-lg shadow-emerald-100 transition-all flex items-center gap-2"
                    >
