...
const InquiryManagement = () => {
  ...
  return (
    <div className="w-full max-w-full px-6 py-6">
      <h2 className="text-2xl font-bold mb-2 text-gray-800">Inquiry Management</h2>
      <p className="text-sm text-gray-600 mb-6">Manage guest inquiries and support messages.</p>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Subject / Message</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                filteredInquiries.map((inquiry) => (
                  <tr key={inquiry.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => openModal(inquiry)}>
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-gray-400">{new Date(inquiry.created_at).toLocaleTimeString()}</div>
                    </td>
                    <td className="px-6 py-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        {inquiry.status !== 'archived' && (
                          <button 
                            ...
                            className="p-1.5 text-gray-500 hover:bg-gray-100 rounded-lg tooltip"
                            title="Archive"
                          >
                        ...
                        <button 
                          ...
                          className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg tooltip"
                          title="Delete"
                        >
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center flex-shrink-0 text-emerald-600 font-bold">
                  {selectedInquiry.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h4 className="font-bold text-gray-900">{selectedInquiry.name}</h4>
                  <div className="flex items-center gap-2 text-sm text-gray-500">
                    <Mail className="w-3 h-3" />
                    <a href={`mailto:${selectedInquiry.email}`} className="hover:text-emerald-600">{selectedInquiry.email}</a>
                  </div>
                  {selectedInquiry.phone && (
                    <div className="flex items-center gap-2 text-sm text-gray-500 mt-1">
                      <Phone className="w-3 h-3" />
                      <a href={`tel:${selectedInquiry.phone}`} className="hover:text-emerald-600">{selectedInquiry.phone}</a>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                <p className="text-xs text-gray-400 mb-2 uppercase tracking-wide font-bold">Message</p>
                <p className="text-gray-700 text-sm whitespace-pre-wrap leading-relaxed">{selectedInquiry.message}</p>
              </div>
