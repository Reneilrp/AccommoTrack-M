...
const PropertyApproval = ({ isEmbedded = false }) => {
  ...
  const fetchProperties = React.useCallback(async (page = 1, status = statusFilter) => {
    setLoading(true);
    setSelectedIds([]); // Clear selection on tab change
    try {
      const res = await adminService.getPropertiesByStatus(status, { page });
      
      if (res.success) {
        setProperties(res.data.items || []);
        if (res.data.pagination) setPagination(res.data.pagination);
      } else {
        showError(res.error || 'Failed to fetch');
      }
    } catch (err) {
