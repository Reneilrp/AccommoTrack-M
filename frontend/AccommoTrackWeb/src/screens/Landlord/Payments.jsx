  ...
  const rows = filteredInvoices.map((inv) => {
    ...
    return (
      <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
        <td className="pl-6 py-4 text-sm font-medium text-gray-900">{invoiceId}</td>
        <td className="px-6 py-4 text-sm text-gray-700">{bookingFromMap?.__derived?.tenant_name || tenantDisplay}</td>
        <td className="px-6 py-4 text-sm text-gray-700">{bookingFromMap?.__derived?.property_title || propertyDisplay}</td>
        <td className="px-6 py-4 text-sm text-gray-700">{bookingFromMap?.__derived?.room_label || roomDisplay}</td>
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="text-center w-full">
            ...
            <p className="text-sm text-gray-500 mt-1">Manage invoices and update booking payment statuses.</p>
          </div>
        </div>
      </header>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-[28rem] pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
              />
            </div>
            ...
            <div className="flex gap-2 flex-wrap ml-2">
              {['all', 'pending', 'paid', 'unpaid', 'partial', 'cancelled'].map((s) => (
                <button
                  ...
                  className={`px-4 py-2.5 rounded-lg font-medium transition-colors ${paymentFilter === s ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <button onClick={() => setShowInvoiceModal(false)} className="text-gray-500 hover:text-gray-700">Close</button>
              </div>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-2">Invoice JSON</p>
                <pre className="bg-gray-50 p-3 rounded text-xs overflow-auto" style={{maxHeight: '40vh'}}>{JSON.stringify(selectedInvoice, null, 2)}</pre>
                <div className="mt-4">
                  <p className="text-sm text-gray-600 mb-2">Fetched Booking (if any)</p>
                  <pre className="bg-gray-50 p-3 rounded text-xs overflow-auto" style={{maxHeight: '40vh'}}>{JSON.stringify(bookingsMap[selectedInvoice.booking_id] || null, null, 2)}</pre>
                </div>
              </div>
              <div className="p-4 border-t border-gray-200 text-right">
                <button onClick={() => setShowInvoiceModal(false)} className="px-4 py-2 bg-gray-100 rounded">Close</button>
              </div>
            </div>
          </div>
        )}
