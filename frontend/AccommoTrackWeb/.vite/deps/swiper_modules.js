import { createElement, elementChildren, elementIndex, elementOffset, elementOuterSize, elementParents, elementTransitionEnd, getDocument, getSlideTransformEl, getTranslate, getWindow, isObject, nextTick, now, setCSSProperty } from "./utils-DfHcR85v.js";
...
function Keyboard(_ref) {
	...
	function handle(event$1) {
		...
		if (document$1.activeElement && document$1.activeElement.nodeName && (document$1.activeElement.nodeName.toLowerCase() === "input" || document$1.activeElement.nodeName.toLowerCase() === "textarea")) return;
...
function Scrollbar(_ref) {
	...
	function init() {
		...
		let el;
		if (typeof params.el === "string" && swiper.isElement) el = swiper.el.querySelector(params.el);
		if (!el && typeof params.el === "string") el = document$1.querySelectorAll(params.el);
...
function A11y(_ref) {
	...
	const init = () => {
		...
		if (hasClickablePagination()) (Array.isArray(swiper.pagination.el) ? swiper.pagination.el : [swiper.pagination.el]).forEach((el) => {
			el.addEventListener("keydown", onEnterOrSpaceKey);
		});
...
function EffectCube(_ref) {
	...
	const setTranslate = () => {
		...
		if (params.shadow) if (isHorizontal) cubeShadowEl.style.transform = `translate3d(0px, ${swiperWidth / 2 + params.shadowOffset}px, ${-swiperWidth / 2}px) rotateX(90deg) rotateZ(0deg) scale(${params.shadowScale})`;
		else {
