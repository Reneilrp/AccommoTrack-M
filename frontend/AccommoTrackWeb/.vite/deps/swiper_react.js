...
import { animateCSSModeScroll, createElement, deleteProps, elementChildren, elementIndex, elementNextAll, elementOuterSize, elementParents, elementPrevAll, elementStyle, extend as extend$1, getDocument, getTranslate, getWindow, nextTick, now, setCSSProperty } from "./utils-DfHcR85v.js";
...
function calcBrowser() {
	...
	return {
		...
		needPerspectiveFix,
		isWebView: /(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/i.test(window$1.navigator.userAgent)
	};
}
function getBrowser() {
...
function updateSlides() {
	...
	for (let i = 0; i < slidesLength; i += 1) {
		slideSize = 0;
		let slide$1;
		if (slides[i]) slide$1 = slides[i];
		if (gridEnabled) swiper.grid.updateSlide(i, slide$1, slidesLength, getDirectionLabel);
		if (slides[i] && elementStyle(slide$1, "display") === "none") continue;
...
function updateSlidesProgress(translate$1) {
	if (translate$1 === void 0) translate$1 = this && this.translate || 0;
	const swiper = this;
	const params = swiper.params;
...
function translateTo(translate$1, speed, runCallbacks, translateBounds, internal) {
	if (translate$1 === void 0) translate$1 = 0;
...
function slideTo(index, speed, runCallbacks, internal, initial) {
	...
	if (rtl && -translate$1 === swiper.translate || !rtl && translate$1 === swiper.translate) {
...
function loopFix(_temp) {
	let { slideRealIndex, slideTo: slideTo$1 = true, direction, setTranslate: setTranslate$1, activeSlideIndex, byController, byMousewheel } = _temp === void 0 ? {} : _temp;
	const swiper = this;
...
function onTouchStart(event) {
	...
	if (edgeSwipeDetection && (startX <= edgeSwipeThreshold || startX >= window$1.innerWidth - edgeSwipeThreshold)) if (edgeSwipeDetection === "prevent") event.preventDefault();
	else return;
	Object.assign(data, {
		isTouched: true,
		isMoved: false,
	...
	if (document$1.activeElement && document$1.activeElement.matches(data.focusableElements) && document$1.activeElement !== targetEl) document$1.activeElement.blur();
...
function onTouchEnd(event) {
	...
	if ([
		...
		if (!(["pointercancel", "contextmenu"].includes(event.type) && (swiper.browser.isSafari || swiper.browser.isWebView))) return;
	}
...
function setBreakpoint() {
	...
	const breakpoint = swiper.getBreakpoint(breakpoints$1, swiper.params.breakpointsBase, swiper.el);
...
function getBreakpoint(breakpoints$1, base, containerEl) {
